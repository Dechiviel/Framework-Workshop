...
@section('content')
    ...
    <div class="relative flex items-center justify-center mx-20">
        <div class="shadow-lg rounded-3xl w-full bg-white p-10">
            <form action="{{ route('salary.update', $salary->id) }}" method="post" class="w-full">
                @csrf
                @method('PUT')

                <div class="flex flex-col justify-start">
                    <h1 class="text-3xl font-bold text-gray-700 mb-6 mt-2">Edit Salary</h1>
                </div>
                ...
                <div class="max-w-full mb-4">
                    <button type="button" id="karyawanButton"
                        ...
                        <span id="selectedKaryawan">{{ $salary->employee->nama_lengkap ?? 'Pilih Karyawan' }}</span>
                    ...
                    <input type="hidden" id="karyawan" name="karyawan_id" value="{{ $salary->employee_id }}">
                </div>
                ...
                <!-- Gaji Pokok -->
                <div class="relative input-container mb-4">
                    <input type="text" id="gaji_pokok_display" required class="input"
                        oninput="formatNumber(this)"
                        value="{{ number_format($salary->gaji_pokok, 0, ',', '.') }}">
                    <label for="gaji_pokok_display" class="placeholder">Gaji Pokok</label>
                    <input type="hidden" id="gaji_pokok" name="gaji_pokok" value="{{ $salary->gaji_pokok }}">
                </div>

                <!-- Tunjangan -->
                <div class="relative input-container mb-4">
                    <input type="text" id="tunjangan_display" required class="input"
                        oninput="formatNumber(this)"
                        value="{{ number_format($salary->tunjangan, 0, ',', '.') }}">
                    <label for="tunjangan_display" class="placeholder">Tunjangan</label>
                    <input type="hidden" id="tunjangan" name="tunjangan" value="{{ $salary->tunjangan }}">
                </div>

                <!-- Potongan -->
                <div class="relative input-container mb-4">
                    <input type="text" id="potongan_display" required class="input"
                        oninput="formatNumber(this)"
                        value="{{ number_format($salary->potongan, 0, ',', '.') }}">
                    <label for="potongan_display" class="placeholder">Potongan</label>
                    <input type="hidden" id="potongan" name="potongan" value="{{ $salary->potongan }}">
                </div>

                <!-- Total Gaji -->
                <div
                    class="flex items-center justify-between w-full px-4 py-3 mb-4 border rounded-xl bg-gray-50 shadow-sm hover:shadow-md transition-all duration-300 ease-in-out">
                    <label for="total_gaji_display" class="text-gray-600 font-medium text-sm">
                        Gaji Total
                    </label>
                    <input class="text-right text-lg font-semibold text-indigo-700 tracking-wide"
                        id="total_gaji_display"
                        readonly
                        value="{{ number_format($salary->total_gaji, 0, ',', '.') }}">
                    <input type="hidden" id="total_gaji" name="total_gaji" value="{{ $salary->total_gaji }}">
                </div>
